...
part 'vod_filter_event.dart';

part 'vod_filter_state.dart';

part 'vod_filter_bloc.freezed.dart';
...
class VodFilterBloc extends Bloc<VodFilterEvent, VodFilterState> {
  VodFilterBloc({required VodRepository vodRepository})
      : _vodRepository = vodRepository,
        super(const VodFilterState()) {
    ...
    on<YearFilterChanged>((event, emit) => _changeYearFilter(event.year, emit),
        transformer: debounce(_duration));

    on<CountryFilterChanged>(
        (event, emit) => _changeCountryFilter(event.countryId, emit),
        transformer: debounce(_duration));

    on<SearchFilterChanged>(
        (event, emit) => _changeSearchFilter(event.keyword, emit),
    ...
    on<ApplyFilter>((event, emit) =>
        _fetchVideos(emit, loadNextPage: event.loadNextPage ?? false));

    on<UndoFilter>((event, emit) => _undoFilter(event, emit));
  }
  final VodRepository _vodRepository;

  void _changeCountryFilter(int countryId, Emitter<VodFilterState> emit) {
    ...
    final List<int> countryIds =
        state.countryIds.where((e) => e != countryId).toList();
  ...
  void _changeCategoryFilter(int categoryId, Emitter<VodFilterState> emit) {
  ...
  void _changePage(Emitter<VodFilterState> emit) {
  ...
  void _undoFilter(UndoFilter event, Emitter<VodFilterState> emit) async {
    emit(
      state.copyWith(
          categoryIds: event.initialCategories,
          countryIds: event.initialCountries),
    );
  }

  void _resetFilter(Emitter<VodFilterState> emit) async {
  ...
  void _filterRemoved(FilterRemoved event, Emitter<VodFilterState> emit) async {
    final int? categoryId = event.categories
        .where((e) => e.title == event.filterName)
        .firstOrNull
        ?.id;

    final int? countryId = event.countries
        .where((e) => e.name == event.filterName)
        .firstOrNull
        ?.id;
    ...
    if (state.releaseYears.contains(int.parse(event.filterName))) {
      emit(state.copyWith(
        releaseYears: state.releaseYears
            .where((e) => e != int.parse(event.filterName))
            .toList(),
      ));
  ...
  Future<void> _changeSearchFilter(
      String search, Emitter<VodFilterState> emit) async {
    emit(
      state.copyWith(
        search: search,
        page: 1,
  ...
  Future<void> _initialize(
      Initialize event, Emitter<VodFilterState> emit) async {
    await _fetchVideos(emit);
  }

  Future<void> _fetchVideos(Emitter<VodFilterState> emit,
      {bool loadNextPage = false}) async {
    final VodSearchParams params = VodSearchParams(
    ...
    emit(
      state.copyWith(
          status: loadNextPage ? Status.loadingNextPage : Status.loading),
    );

    final ResponseModel<VodListModel> response =
        await _vodRepository.search(params);
    (switch (response) {
      SuccessModel<VodListModel>() => {
